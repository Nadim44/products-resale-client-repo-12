...
const AllUsers = () => {
    ...
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://assignment-12-server-nu.vercel.app/users');
            const data = await res.json();
            return data;
        }
    })
    const handleMakeAdmin = id => {
        fetch(`https://assignment-12-server-nu.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
    ...
    const handleDeleteUser = user => {
        // console.log(user)
        fetch(`https://assignment-12-server-nu.vercel.app/users/${user._id}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
